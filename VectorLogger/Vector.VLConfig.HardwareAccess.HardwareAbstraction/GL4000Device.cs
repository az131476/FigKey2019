...
{
	internal class GL4000Device : LoggerDeviceWindowsFileSystem, IGL4000Device
	{
		...
		public override ILoggerSpecifics LoggerSpecifics
		{
			get
			{
				return GL4000Scanner.LoggerSpecifics;
			}
		}
		...
		public GL4000Device(string hardwareKey, bool isOnline)
		{
		...
		{
			GL4000ctrl gL4000ctrl = new GL4000ctrl();
			string message = "";
			if (!gL4000ctrl.SetRealTimeClock(base.HardwareKey, out message))
			{
		...
		public override bool SetVehicleName(string name)
		{
			...
			if (!gL4000ctrl.SetVehicleName(name, out message))
			{
				InformMessageBox.Error(message);
				return false;
			}
