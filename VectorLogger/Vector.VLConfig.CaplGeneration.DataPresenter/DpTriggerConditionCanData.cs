...
{
	internal class DpTriggerConditionCanData : DpTriggerCondition
	{
		private readonly CANDataEvent mEvent;
		...
		public string MsgConditionString
		{
			get
			{
				return CaplHelper.MakeConditionStringCanId(CondRelation.Equal, "this.ID", this.mEvent.ID.Value, 0u, this.mEvent.IsExtendedId.Value);
			}
		}

		public DpTriggerConditionCanData(uint triggerIdent, uint conditionIdent, Event conditionEvent) : base(triggerIdent, conditionIdent, conditionEvent)
		{
			this.mEvent = (conditionEvent as CANDataEvent);
		}
	}
}
